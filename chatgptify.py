...
def spotify_connected() -> bool:
    return not not os.environ.get('SPOTIFY_CLIENT_ID')
...
class SpotifyPlaylist():
    ...
    def save_playlist(self, name: str = "") -> str:
        ...
        if not spotify_connected():
            print("Spotify not connected, will not export playlist to spotify...")
            return

        user_id = self.sp.current_user()['id']
        new_playlist = self.sp.user_playlist_create(user=user_id, name=name, public=True)
        ...
        self.sp.user_playlist_add_tracks(
            user=user_id, playlist_id=p_id, tracks=tracks)
